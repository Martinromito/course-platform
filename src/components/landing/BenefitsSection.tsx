...
export default function BenefitsSection() {
  ...
  return (
    <section id="beneficios" className="py-16 sm:py-24 bg-[#FAF9F6]">
      <div className="max-w-7xl mx-auto px-5 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
          {benefits.map((b, i) => (
            <div
              key={i}
              className="group relative p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-[#E5E0D8] bg-white hover:border-[#b04b2b]/50 hover:shadow-xl transition-all duration-300"
            >
              <div className="relative flex sm:block items-start gap-4">
                ...
                <div>
                  <h3 className="text-[#1A1A1A] font-bold text-lg sm:text-xl mb-2 sm:mb-3">{b.title}</h3>
                  <p className="text-[#705E45] leading-relaxed text-sm sm:text-base">{b.description}</p>
                </div>
              </div>
            </div>
          ))}
